...
class ApiHttpIntegrationTests(unittest.TestCase):
    ...
    def test_backtest_report_returns_walk_forward_metrics(self):
        ...
        self.assertEqual(len(payload["shadow_comparison"]["threshold_checks"]), 7)
        self.assertEqual(len(payload["shadow_comparison"]["regime_breakdown"]), 2)
        self.assertIn(payload["shadow_comparison"]["threshold_checks"][0]["status"], {"pass", "fail", "warming_up"})
